      ...
      <Image src={product.image} alt={product.name} width={500} height={500} className="product-image" />
        ...
        <div className="options">
          ...
          <div className="option-group">
            <button className={type === "Hot" ? "option selected" : "option"} onClick={() => setType("Hot")}>Hot</button>
            {product.TypeOfDrinks.includes("Cold") && (
              <button className={type === "Cold" ? "option selected" : "option"} onClick={() => setType("Cold")}>Cold</button>
            )}
          </div>

          <h2>Add on</h2>
          <div className="option-group">
            <button className={addOn === "None" ? "option selected" : "option"} onClick={() => setAddOn("None")}>None</button>
            <button className={addOn === "Oat Milk" ? "option selected" : "option"} onClick={() => setAddOn("Oat Milk")}>Oat Milk</button>
            <button className={addOn === "Brown Sugar Jelly" ? "option selected" : "option"} onClick={() => setAddOn("Brown Sugar Jelly")}>Brown Sugar Jelly</button>
          </div>

          <h2>Choice of Sweetness</h2>
          <div className="option-group">
            <button className={sweetness === "0%" ? "option selected" : "option"} onClick={() => setSweetness("0%")}>0%</button>
            <button className={sweetness === "30%" ? "option selected" : "option"} onClick={() => setSweetness("30%")}>30%</button>
            <button className={sweetness === "50%" ? "option selected" : "option"} onClick={() => setSweetness("50%")}>50%</button>
            <button className={sweetness === "75%" ? "option selected" : "option"} onClick={() => setSweetness("75%")}>75%</button>
            <button className={sweetness === "100%" ? "option selected" : "option"} onClick={() => setSweetness("100%")}>100%</button>
          </div>

          <div className="price">
            <h3>Total Price: {type === "Hot" ? product.hotPrice : product.coldPrice}.-</h3>
          </div>
